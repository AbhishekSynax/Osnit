...
API_URL = "https://numinfosource.alphaapi.workers.dev/?key=anshapi&mobile="  # Updated API URL
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats_text = (
        ...
        f"📈 Active Today: {len([u for u in users.values() if 'joined_at' in u and datetime.datetime.fromisoformat(u['joined_at']).date() == datetime.date.today()])}"
    )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success") and result.get("result") and len(result["result"]) > 0:
                    ...
                    # Parse address with special handling for the format with !00!
                    address_parts = data.get("address", "").split("!")
                    # Filter out empty parts and !00! markers
                    address_parts = [part for part in address_parts if part and part != "00"]
