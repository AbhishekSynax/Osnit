...
GROUPS_FILE = "groups.json"  # New file for group management
...
# Aadhar to Family Info API
AADHAR_FAMILY_API_URL = "https://darkietech.site/numapi.php?action=api&key=CODER&aadhar_family="
...
AADHAR_FAMILY_IMAGE = "https://i.ibb.co/yn6kGsjg/file-000000004be87207bbc4d85e8782b51a.png"  # Image for Aadhar to Family Info
...
ACTIVATE_GROUP = range(1)  # For activating groups from admin panel
...
def load_credit_costs() -> dict:
    ...
    if not costs:
        costs = {
            ...
            "aadhar_family_search": 2  # Aadhar to Family Info
        }
...
def clear_service_states(context: ContextTypes.DEFAULT_TYPE):
    ...
    service_states = [
        ...
        "awaiting_aadhar"  # Added for Aadhar to Family Info
    ]
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("✨ Stylish Text", callback_data="stylish_text"),InlineKeyboardButton("👪 Aadhar Family Info", callback_data="aadhar_family_search")]
    ]
...
# ================= AADHAR FAMILY HELPER FUNCTIONS =================
def format_aadhar_family_data(data: dict) -> str:
    """Format Aadhar family data for display"""
    if not data:
        return "❌ Failed to fetch Aadhar family data. Please try again."
    
    # Extract data with defaults
    search_type = data.get("search_type", "N/A")
    ration_card_number = data.get("ration_card_number", "N/A")
    session_time = data.get("session_time", "N/A")
    family_data = data.get("data", {})
    
    # Extract family details
    ration_card_id = family_data.get("ration_card_id", "N/A")
    card_type = family_data.get("card_type", "N/A")
    scheme = family_data.get("scheme", "N/A")
    issue_date = family_data.get("issue_date", "N/A")
    state = family_data.get("state", "N/A")
    district = family_data.get("district", "N/A")
    address = family_data.get("address", "N/A")
    fps_code = family_data.get("fps_code", "N/A")
    fps_name = family_data.get("fps_name", "N/A")
    members = family_data.get("members", [])
    
    # Format message
    msg = (
        f"👪 *Aadhar Family Information*\n\n"
        f"🔍 *Search Type:* {search_type}\n"
        f"🆔 *Ration Card Number:* `{ration_card_number}`\n"
        f"🕐 *Session Time:* {session_time}\n\n"
        f"📋 *Ration Card Details*\n"
        f"🆔 *Card ID:* `{ration_card_id}`\n"
    ...
    if members and len(members) > 0:
        ...
        for i, member in enumerate(members, 1):
            ...
            uid_masked = member.get("uid_masked", "N/A")
            relationship = member.get("relationship", "N/A")
            ekyc_status = member.get("ekyc_status", "N/A")
            
            gender_icon = "👨" if gender == "M" else "👩" if gender == "F" else "👤"
            ekyc_status_icon = "✅" if ekyc_status == "Y" else "❌"
            
            msg += (
                f"{i}. {gender_icon} *Name:* {member_name}\n"
                f"   🆔 *Member ID:* `{member_id}`\n"
                f"   🆔 *UID:* `{uid_masked}`\n"
                f"   🤝 *Relationship:* {relationship}\n"
                f"   📱 *eKYC Status:* {ekyc_status_icon} {ekyc_status}\n\n"
            )
    ...
    try:
        ...
        if top_features:
            ...
            feature_names = {
                ...
                "aadhar_family_search": "👪 Aadhar Family"
            }
...
async def activate_group_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /activate command in groups - ONLY BOT ADMIN CAN USE"""
    ...
    if user_id != str(ADMIN_ID):
        await update.message.reply_text("❌ Only the bot admin can activate groups. Use /admin to access the admin panel.")
        return
    ...
    if activate_group(chat_id, user_id):
        await update.message.reply_text(
            ...
            "Bot commands will now work in this group without any buttons or ads.\n\n"
            "Available commands:\n"
            "/number - Search mobile number info\n"
            "/vehicle - Search vehicle RC info\n"
            "/aadhar - Search Aadhar family info\n"
            "/pincode - Search pincode info\n"
            "/ip - Search IP info\n"
            "/numname - Search number to name\n"
            "/instagram - Search Instagram info\n"
            "/reel - Download Instagram Reel\n"
            "/spotify - Search Spotify music\n"
            "/freefire - Search Free Fire info\n"
            "/gst - Search GST info\n"
            "/faceswap - Face swap\n"
            "/tts - Text to voice\n"
            "/youtube - YouTube downloader\n"
            "/stylish - Generate stylish text\n\n"
            "Made by @synaxnetwork",
            parse_mode="Markdown"
        )
    else:
...
async def deactivate_group_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /deactivate command in groups - ONLY BOT ADMIN CAN USE"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        await update.message.reply_text("❌ This command can only be used in groups.")
        return
    ...
    if user_id != str(ADMIN_ID):
        await update.message.reply_text("❌ Only the bot admin can deactivate groups. Use /admin to access the admin panel.")
        return
...
async def number_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /number command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if not number.isdigit() or len(number) != 10:
        await update.message.reply_text(
            "❌ Invalid number! Please send a 10-digit mobile number.\n\n"
            "Usage: /number 9876543210",
            parse_mode="Markdown"
        )
        return
    ...
    # Skip credit check for activated groups
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Number search requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("success") and result.get("result"):
                ...
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                ...
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("number_search"))
                await update.message.reply_text("❌ No record found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("number_search"))
            await update.message.reply_text("❌ API error!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
            
    except Exception as e:
        # Refund credit if not premium and not in active group
        if not is_group_active(chat_id) and not is_premium_user(user_id):
            add_credits(user_id, get_credit_cost("number_search"))
        await update.message.reply_text("❌ Search failed!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        logger.error(f"Search error: {e}")

async def vehicle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /vehicle command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    # Check if user provided a vehicle number
    if not context.args:
        await update.message.reply_text(
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Vehicle search requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                ...
                msg += "🏢 *Group Active - Unlimited Searches*"
            
            await update.message.reply_text(msg, parse_mode="Markdown")
        else:
            # Refund credits if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("vehicle_search"))
            await update.message.reply_text("❌ No record found!" + (" Credits refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
            
    except Exception as e:
        # Refund credits if not premium and not in active group
        if not is_group_active(chat_id) and not is_premium_user(user_id):
            add_credits(user_id, get_credit_cost("vehicle_search"))
        await update.message.reply_text("❌ Search failed!" + (" Credits refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        logger.error(f"Vehicle search error: {e}")

async def aadhar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /aadhar command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    # Check if user provided an Aadhar number
    if not context.args:
        await update.message.reply_text(
            "❌ Please provide an Aadhar number.\n\n"
            "Usage: /aadhar 123456789012",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        # Check if user is premium or has enough credits
        credit_cost = get_credit_cost("aadhar_family_search")
        if not is_premium_user(user_id):
            user = get_user(user_id)
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Aadhar family search requires {credit_cost} credits. Please buy more credits."
                )
                return
            ...
            remove_credits(user_id, credit_cost)
            # Update statistics
            update_search_stats("aadhar_family_search", credit_cost)
    
    # Perform API search
    try:
        response = requests.get(
            f"{AADHAR_FAMILY_API_URL}{aadhar_number}",
            headers={"User-Agent": "Mozilla/5.0"},
            timeout=15
        )
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                # Format the result
                result_text = format_aadhar_family_data(data)
                ...
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("aadhar_family_search"))
                await update.message.reply_text("❌ No record found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
        ...
        if not is_group_active(chat_id) and not is_premium_user(user_id):
            add_credits(user_id, get_credit_cost("aadhar_family_search"))
        await update.message.reply_text("❌ Search failed!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        logger.error(f"Aadhar family search error: {e}")

async def pincode_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /pincode command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    # Check if user provided a pincode
    if not context.args:
        await update.message.reply_text(
            "❌ Please provide a pincode.\n\n"
            "Usage: /pincode 110001",
            parse_mode="Markdown"
        )
        return
    ...
    if not pincode.isdigit() or len(pincode) != 6:
        await update.message.reply_text(
            "❌ Invalid pincode! Please send a 6-digit Indian pincode.\n\n"
            "Usage: /pincode 110001",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Pincode search requires {credit_cost} credits. Please buy more credits."
                )
                return
        ...
        if response.status_code == 200:
            ...
            if data and len(data) > 0 and data[0].get("Status") == "Success":
                ...
                if post_offices and len(post_offices) > 0:
                    ...
                    # Refund credit if not premium and not in active group
                    if not is_group_active(chat_id) and not is_premium_user(user_id):
                        add_credits(user_id, get_credit_cost("pincode_search"))
                    await update.message.reply_text("❌ No post offices found for this pincode!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
            else:
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("pincode_search"))
                await update.message.reply_text("❌ Invalid pincode or no data found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("pincode_search"))
...
async def ip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(ip_parts) != 4 or not all(part.isdigit() and 0 <= int(part) <= 255 for part in ip_parts):
        await update.message.reply_text(
            "❌ Invalid IP address! Please send a valid IP address.\n\n"
            "Usage: /ip 8.8.8.8",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! IP info search requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    try:
        response = requests.get(
            f"{IP_INFO_API_URL}?ip={ip_address}",
            headers={"User-Agent": "Mozilla/5.0"},
            timeout=15
        )
        ...
        if response.status_code == 200:
            ...
            # Refund credits if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("ip_info_search"))
            await update.message.reply_text("❌ No record found!" + (" Credits refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
...
async def numname_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /numname command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ Please provide a mobile number with country code.\n\n"
            "Usage: /numname 919065146522",
            parse_mode="Markdown"
        )
        return
    ...
    if not number.isdigit():
        await update.message.reply_text(
            "❌ Invalid number! Please send a valid mobile number with country code.\n\n"
            "Usage: /numname 919065146522",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Number to name search requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("success") and result.get("data") and result["data"].get("success"):
                ...
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("num_name_search"))
                await update.message.reply_text("❌ No record found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
...
async def instagram_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /instagram command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if username.startswith("@"):
        username = username[1:]  # Remove @ if user included it
    
    if not username or len(username) < 1:
        await update.message.reply_text(
            "❌ Invalid username! Please send a valid Instagram username (without @).\n\n"
            "Usage: /instagram username",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Instagram info search requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data and "id" in data:
                ...
                await update.message.reply_text(profile_data, parse_mode="HTML")
            else:
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("instagram_search"))
                await update.message.reply_text("❌ Profile not found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("instagram_search"))
        ...
        if not is_group_active(chat_id) and not is_premium_user(user_id):
            add_credits(user_id, get_credit_cost("instagram_search"))
        await update.message.reply_text(f"❌ Error: {str(e)}" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        logger.error(f"Instagram search error: {e}")

async def reel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /reel command in groups for Instagram Reel download"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "Usage: /reel https://www.instagram.com/reel/...",
            parse_mode="Markdown"
        )
        return
    ...
    # Validate URL
    if not url.startswith("https://www.instagram.com/reel/") and not url.startswith("https://instagram.com/reel/"):
        await update.message.reply_text(
            "❌ Invalid Instagram Reel URL! Please send a valid Instagram Reel URL.",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Instagram Reel download requires {credit_cost} credits. Please buy more credits."
                )
                return
        ...
        if response.status_code == 200:
            ...
            if not data.get("error") and data.get("result"):
                ...
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    ...
                    msg += "🏢 *Group Active - Unlimited Downloads*"
                ...
                if download_url:
                    ...
                    # Refund credit if not premium and not in active group
                    if not is_group_active(chat_id) and not is_premium_user(user_id):
                        add_credits(user_id, get_credit_cost("instagram_reel_download"))
                    await update.message.reply_text("❌ Download URL not found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
            else:
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("instagram_reel_download"))
                await update.message.reply_text("❌ Reel not found or private!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("instagram_reel_download"))
...
async def spotify_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /spotify command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    # Check if user provided a song name or URL
    if not context.args:
        await update.message.reply_text(
    ...
    # Check if it's a URL
    is_url = query.startswith("https://open.spotify.com/track/")
    ...
    if not is_group_active(chat_id):
        # Check if user is premium or has enough credits
        credit_cost = get_credit_cost("spotify_search" if not is_url else "spotify_url_search")
        if not is_premium_user(user_id):
            user = get_user(user_id)
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Spotify search requires {credit_cost} credits. Please buy more credits."
                )
                return
            ...
            remove_credits(user_id, credit_cost)
            # Update statistics
            update_search_stats("spotify_search" if not is_url else "spotify_url_search", credit_cost)
    
    # Send processing message
    processing_msg = await update.message.reply_text(
        f"*Please Wait...* Searching for: `{query[:30]}...`",
        parse_mode="Markdown"
    )
        ...
        if response.status_code == 200:
            ...
            if data.get("status"):
                ...
                if download_url:
                    ...
                    # Refund credit if not premium and not in active group
                    if not is_group_active(chat_id) and not is_premium_user(user_id):
                        add_credits(user_id, get_credit_cost("spotify_search" if not is_url else "spotify_url_search"))
                    await update.message.reply_text("❌ Download URL not found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
            else:
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("spotify_search" if not is_url else "spotify_url_search"))
                await update.message.reply_text("❌ Song not found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("spotify_search" if not is_url else "spotify_url_search"))
...
async def freefire_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /freefire command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if not uid.isdigit():
        await update.message.reply_text(
            "❌ Invalid UID! Please send a valid Free Fire UID (numbers only).\n\n"
            "Usage: /freefire 6662300192",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Free Fire info search requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data and "basicinfo" in data:
                ...
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("freefire_search"))
                await update.message.reply_text("❌ No record found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("freefire_search"))
            await update.message.reply_text("❌ API error!" + (" Credits refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
...
async def gst_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if user provided a GST number
    if not context.args:
        await update.message.reply_text(
            "❌ Please provide a GST number.\n\n"
            "Usage: /gst 19BOKPS7056D1ZI",
            parse_mode="Markdown"
        )
        return
    ...
    if not gst_number or len(gst_number) != 15 or not gst_number.isalnum():
        await update.message.reply_text(
            "❌ Invalid GST number! Please send a valid 15-digit GST number.\n\n"
            "Usage: /gst 19BOKPS7056D1ZI",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! GST search requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("status") == "success" and data.get("data"):
                ...
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("gst_search"))
                await update.message.reply_text("❌ No record found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else ""))
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
...
async def faceswap_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /faceswap command in groups"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Face swap requires {credit_cost} credits. Please buy more credits."
                )
                return
...
async def tts_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /tts command in groups for Text to Voice"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ Please provide text to convert to voice.\n\n"
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Text to voice requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    try:
            ...
            if not data.get("error") and data.get("audio"):
                ...
                try:
                    ...
                    if not is_group_active(chat_id) and not is_premium_user(user_id):
                        ...
                        caption += "🏢 *Group Active - Unlimited Conversions*"
                    ...
                    # Refund credit if not premium and not in active group
                    if not is_group_active(chat_id) and not is_premium_user(user_id):
                        add_credits(user_id, get_credit_cost("text_to_voice"))
                    logger.error(f"Error processing audio: {e}")
                    await update.message.reply_text(
                        "❌ Failed to process audio!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else "")
                    )
            else:
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("text_to_voice"))
                await update.message.reply_text(
                    "❌ Failed to convert text to voice!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else "")
                )
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
                add_credits(user_id, get_credit_cost("text_to_voice"))
            await update.message.reply_text(
                "❌ API error!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else "")
            )
        ...
        if not is_group_active(chat_id) and not is_premium_user(user_id):
            add_credits(user_id, get_credit_cost("text_to_voice"))
        await update.message.reply_text(
            f"❌ Error: {str(e)}" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else "")
        )
        logger.error(f"Text to voice error: {e}")

async def youtube_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /youtube command in groups for YouTube download"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if not (url.startswith("https://www.youtube.com/watch?v=") or url.startswith("https://youtu.be/") or url.startswith("https://m.youtube.com/watch?v=")):
        await update.message.reply_text(
            "❌ Invalid YouTube URL! Please send a valid YouTube video URL.",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! YouTube download requires {credit_cost} credits. Please buy more credits."
                )
                return
        ...
        if response.status_code == 200:
            ...
            if data.get("status") and data.get("video"):
                ...
                if video_to_send:
                    ...
                    # Refund credit if not premium and not in active group
                    if not is_group_active(chat_id) and not is_premium_user(user_id):
                        add_credits(user_id, get_credit_cost("youtube_download"))
                    await update.message.reply_text(
                        "❌ Download URL not found!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else "")
                    )
            else:
                # Refund credit if not premium and not in active group
                if not is_group_active(chat_id) and not is_premium_user(user_id):
                    add_credits(user_id, get_credit_cost("youtube_download"))
                await update.message.reply_text(
                    "❌ Video not found or private!" + (" Credit refunded." if not is_group_active(chat_id) and not is_premium_user(user_id) else "")
                )
        else:
            # Refund credit if not premium and not in active group
            if not is_group_active(chat_id) and not is_premium_user(user_id):
...
async def stylish_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /stylish command in groups for stylish text generation"""
    # Check if command is used in a group
    if update.effective_chat.type not in ["group", "supergroup"]:
        return
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ Please provide text to convert to stylish text.\n\n"
            "Usage: /stylish Your text here",
            parse_mode="Markdown"
        )
        return
    ...
    if not is_group_active(chat_id):
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_text(
                    f"❌ Insufficient credits! Stylish text generation requires {credit_cost} credits. Please buy more credits."
                )
                return
    ...
    status_text = ""
    if not is_group_active(chat_id) and not is_premium_user(user_id):
        status_text = f"💰 Remaining Credits: {get_user(user_id)['balance']}"
    elif is_premium_user(user_id):
        status_text = "👑 Premium User - Unlimited Generations"
    else:
        status_text = "🏢 Group Active - Unlimited Generations"
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "aadhar_family_search":
        # Clear all service states before starting Aadhar family search
        clear_service_states(context)
        context.user_data["awaiting_aadhar"] = True
        await query.message.reply_photo(
            photo=AADHAR_FAMILY_IMAGE,
            caption="👪 *Aadhar to Family Info*\n\nPlease send an Aadhar number to get family information:\n\nExample: `123456789012`",
            parse_mode="Markdown"
        )
    ...
    # Group activation from admin panel
    ...
    # Group deactivation from admin panel
...
async def show_group_management(query, context):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📋 All Groups", callback_data="admin_all_groups")],
...
async def show_all_groups(query, context):
    ...
    for group_id, group_data in groups.items():
        status = "✅ Active" if group_data.get("active", False) else "❌ Inactive"
        admin_id = group_data.get("admin_id", "N/A")
        activated_at = group_data.get("activated_at", "N/A")
...
async def show_credit_costs(query, context):
    ...
    cost_text = (
        ...
        f"👪 Aadhar Family Info: {costs.get('aadhar_family_search', 2)} credits\n\n"
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("👪 Aadhar Family", callback_data="credit_cost_aadhar_family_search")],
        [InlineKeyboardButton("🔙 Back to Admin", callback_data="admin_stats")]
    ]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "activate_group":
            ...
            if group_id.startswith("-100") and group_id[1:].isdigit():
                if activate_group(group_id, str(ADMIN_ID)):
                    await update.message.reply_text(
                        f"✅ *Group Activated Successfully*\n\n"
                        f"Group ID: `{group_id}`\n\n"
                        f"Bot commands will now work in this group without any buttons or ads.",
                        parse_mode="Markdown"
                    )
                else:
                    await update.message.reply_text("❌ Failed to activate group. Please try again.")
                ...
                await update.message.reply_text(
                    "❌ Invalid Group ID! Please send a valid group ID (e.g., -1001234567890).",
                    parse_mode="Markdown"
                )
            del context.user_data["admin_action"]
            return
            ...
            if group_id.startswith("-100") and group_id[1:].isdigit():
                if deactivate_group(group_id):
                    await update.message.reply_text(
                        f"✅ *Group Deactivated Successfully*\n\n"
                        f"Group ID: `{group_id}`\n\n"
                        f"Bot commands will no longer work in this group.",
                        parse_mode="Markdown"
                    )
                else:
                    await update.message.reply_text("❌ Failed to deactivate group. Please try again.")
    ...
    # Handle Aadhar family search
    if context.user_data.get("awaiting_aadhar"):
        context.user_data["awaiting_aadhar"] = False
        
        # Validate Aadhar number (basic validation - should be 12 digits)
        if not text or not text.isdigit() or len(text) != 12:
            await update.message.reply_photo(
                photo=AADHAR_FAMILY_IMAGE,
                caption="❌ Invalid Aadhar number! Please send a valid 12-digit Aadhar number."
            )
            return
        
        # Send processing message
        processing_msg = await update.message.reply_text(
            f"*Please Wait...* Processing your request for Aadhar number `{text}`",
            parse_mode="Markdown"
        )
        ...
        if not is_premium_user(user_id):
            ...
            if user["balance"] < credit_cost:
                await update.message.reply_photo(
                    photo=AADHAR_FAMILY_IMAGE,
                    caption=f"❌ Insufficient credits! Aadhar family search requires {credit_cost} credits. Please buy more credits."
                )
                return
...
def main():
    ...
    application.add_handler(CommandHandler("numname", numname_command))
    ...
    application.add_handler(CommandHandler("freefire", freefire_command))
    application.add_handler(CommandHandler("gst", gst_command))
    application.add_handler(CommandHandler("faceswap", faceswap_command))
